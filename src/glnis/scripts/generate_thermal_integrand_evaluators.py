...
def run_generate_thermal_integrand_evaluators(force_rebuild: bool = False) -> None:
    ...
    for example, path_to_example in examples.items():
        ...
        before = perf_counter()
        shell_print(f"Generating example {Colour.YELLOW}{example}{Colour.END}: {p}")
        kintegrands.SymbolicaIntegrand(params=params, path_to_example=p, force_rebuild=force_rebuild)
        after = perf_counter()
        shell_print(
            f"Finished generating example {Colour.YELLOW}{example}{Colour.END} in {Colour.BLUE}{after - before:.2f}s{Colour.END}")
